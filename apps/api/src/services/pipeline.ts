...
import { dirs, fileSizeMb, writeJob, writeReport } from './storage.js';
...
export async function startAnalysis(file: MultipartFile, metadataRaw: string, optionsRaw: string) {
  ...
  await writeJob(id, { id, status: 'processing', stage: 'ingest', metadata, options, uploadPath, createdAt, updatedAt: createdAt });
  processAnalysis(id, uploadPath, metadata, options, createdAt).catch(console.error);
  return { id, status: 'processing' as const };
}

async function processAnalysis(id: string, uploadPath: string, metadata: any, options: any, createdAt: string) {
  let confidence = 1;
  const internal_warnings: string[] = [];
  const setStage = (stage: string) => writeJob(id, {
    id,
    status: 'processing',
    stage,
    metadata,
  ...
  if ((features.duration_s ?? 0) / 60 > env.MAX_DURATION_MIN) {
    await writeJob(id, {
      id,
      status: 'failed',
      error: `Audio exceeds MAX_DURATION_MIN=${env.MAX_DURATION_MIN}`,
      createdAt,
      updatedAt: new Date().toISOString()
    });
    return;
  ...
  // Upload retention is now managed by TTL cleanup scheduler.
}
